...
import torch
import time
from utils.data_utils import get_loaders, test_ppl, layerwise_test_ppl
...
from utils.quant_utils import wrap_to_quant_model, init_weight_quantizer, init_input_quantizer, register_online_had, get_act_stat, init_k_quantizer, init_v_quantizer, init_q_quantizer, init_a_quantizer, apply_naive_sdpa, get_quant_config,check_quantizer
...
def main():
    ...
    parser.add_argument("--q_bits", type=int, default=16,help="") # [added]
    parser.add_argument("--a_bits", type=int, default=16,help="")
    ...
    # ------------------ spike ---------------------------------------------
    parser.add_argument("--T", type=int, default=512, help="Number of time steps.")
    ...
    if args.resume_quant:
        ...
        if 'pythia' in args.model_path:
            model = GPTNeoXForCausalLM.from_pretrained(args.model_path, config=config, device_map='cpu',torch_dtype=dtype,trust_remote_code=True)
            model.rename()
        else:
        ...
        if args.set_prefixed_tokens or include_static:
            ...
            if args.a_bits < 16:
                layers = model_utils.get_layers(model)
                for layer in layers:
                    apply_naive_sdpa(layer.self_attn)
        ...
        if args.q_bits < 16:
            ...
            init_q_quantizer(args, model, activation_stat)

        # [added] init softmax quantizer
        if args.a_bits < 16:
            logger.info('init a quantizer')
            init_a_quantizer(args, model, activation_stat)
        ...
        #     hooks.append(layer.self_attn.softmax.register_forward_hook(avg_after_hook))
