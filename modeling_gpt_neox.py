...
from transformers.models.deprecated._archive_maps import GPT_NEOX_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
class GPTNeoXAttention(nn.Module):
    ...
    def rename(self):
        ...
        qkv_weight = self.query_key_value.weight.data.transpose(0, 1).view(self.hidden_size, self.num_attention_heads, 3 * self.head_size)
        qkv_bias = self.query_key_value.bias.data.view(self.num_attention_heads, 3 * self.head_size)
        qkv_weight_split = lambda s: s.flatten(-2, -1).transpose(0, 1).contiguous()
        qkv_bias_split = lambda s: s.flatten().contiguous()
        q_weight, k_weight, v_weight = map(qkv_weight_split, qkv_weight.split(self.head_size, dim=-1))
        q_bias, k_bias, v_bias = map(qkv_bias_split, qkv_bias.split(self.head_size, dim=-1))
        self.q_proj = nn.Linear(self.hidden_size, self.hidden_size)
        self.k_proj = nn.Linear(self.hidden_size, self.hidden_size)
        self.v_proj = nn.Linear(self.hidden_size, self.hidden_size)
        self.q_proj.weight.data = q_weight.clone().contiguous()
        self.k_proj.weight.data = k_weight.clone().contiguous()
        self.v_proj.weight.data = v_weight.clone().contiguous()
        self.q_proj.bias.data = q_bias.clone().contiguous()
        self.k_proj.bias.data = k_bias.clone().contiguous()
        self.v_proj.bias.data = v_bias.clone().contiguous()
    ...
    def forward(
        ...
        query = query.view(*query.shape[:-1], self.num_attention_heads, self.head_size).permute(0, 2, 1, 3)
        key = key.view(*key.shape[:-1], self.num_attention_heads, self.head_size).permute(0, 2, 1, 3)
        value = value.view(*value.shape[:-1], self.num_attention_heads, self.head_size).permute(0, 2, 1, 3)

        # Compute token offset for rotary embeddings (when decoding)
        seq_len = key.shape[-2]
        if has_layer_past:
            # seq_len += layer_past[0].shape[-2]
            seq_len += past_key_value._seen_tokens
        cos, sin = self.rotary_emb(value, seq_len=seq_len)
        query, key = apply_rotary_pos_emb(query, key, cos, sin, position_ids, self.rotary_ndims)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, rotary_ndims, unsqueeze_dim=1):
    ...
    q_embed = torch.cat((q_embed, q_pass), dim=-1).contiguous()
    k_embed = torch.cat((k_embed, k_pass), dim=-1).contiguous()
...
class GPTNeoXMLP(nn.Module):
    ...
    def rename(self):
        self.up_proj = self.dense_h_to_4h
        self.act_fn = self.act
        self.down_proj = self.dense_4h_to_h
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def rename(self):
        self.embed_tokens = self.embed_in
        self.norm = self.final_layer_norm
        del self.embed_in
