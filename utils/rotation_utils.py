...
def fuse_layer_norms(model):
    ...
    for layer in layers:
        ...
        elif model_type == model_utils.PYTHIA_MODEL:
...
def rotate_attention_inputs(layer, Q, model_type) -> None:
    ...
    attention_inputs = [layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj]
    for W in attention_inputs:
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    if model_type == model_utils.LLAMA_MODEL or model_type == model_utils.QWEN2_MODEL or model_type == model_utils.MISTRAL_MODEL or model_type == model_utils.PYTHIA_MODEL:
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.use_k_quant and self.k_bits < 16:
            ...
            # [commment] quantization happens on the whole channels, not on every head
            k = self.k_quantizer(k).reshape((bsz, seq_len, num_heads, head_dim)).transpose(1, 2).to(q)

        # [added]
        # from einops import rearrange
        # print('before quant: ', rearrange(q, '(T B) ... -> T B ...', T=8).mean(0))
        # print('before quant: ', q)
        # torch.save(q, 'input.pt')
        # import joblib
        # joblib.dump(self.q_quantizer, 'q_quantizer.joblib')
        if self.use_q_quant and self.q_bits < 16:
            q = q.transpose(1, 2).flatten(-2)
            q = self.q_quantizer(q).reshape((bsz, seq_len, num_heads, head_dim)).transpose(1, 2).to(k)
        # print('after quant: ', rearrange(q, '(T B) ... -> T B ...', T=8).mean(0))
        # print('after quant: ', q)
        # # torch.save(q, 'output.pt')
        return q, k
