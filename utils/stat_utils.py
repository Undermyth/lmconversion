...
def stat_layer_wise_outlier_token_number(dataloader, output_activation, model, outlier_threshold=50, outlier_object='hidden_state'):
    # [comment] each token is represented by the maximum absolute value in all dimensions
    # [comment] by this value, if a token is higher than median by a ratio, it is considered an outlier
    stats = []
    data_num = len(dataloader)
    for i in range(data_num):
        data = dataloader[i][0]
...
def stat_outlier_token_position(dataloader, output_activation, model, prefixed_tokens=None, outlier_threshold=20, outlier_object='hidden_state'):
    stats = []
    data_num = len(dataloader)
    model_name = 'model' if not isinstance(model, GPTNeoXForCausalLM) else 'gpt_neox'
    downproj_name = 'down_proj' if not isinstance(model, GPTNeoXForCausalLM) else 'dense_4h_to_h'
    for i in range(data_num):
        data = dataloader[i][0]
        if prefixed_tokens is not None:
            data = torch.cat([torch.tensor([prefixed_tokens]),data],dim=1)
        with torch.no_grad():
            model(data.to('cuda'))
        num_layers = len(model.model.layers) if not isinstance(model, GPTNeoXForCausalLM) else len(model.gpt_neox.layers)
        for block_index in range(num_layers):
            if outlier_object == 'hidden_state':
                entire_name = f'{model_name}.layers.{block_index}'
            elif outlier_object == 'down_proj':
                entire_name = f'{model_name}.layers.{block_index}.mlp.{downproj_name}'
            else:
                raise NotImplementedError
            activation_abs = output_activation[entire_name].abs()
            activation_abs = activation_abs.max(dim=-1).values
...
def get_prefixed_tokens(dataloader, model, tokenizer, model_name, outlier_threshold=64, activation_type='down_proj'):
    ...
    # [comment] stats: List[TensorType[1, "num_layers", torch.int]]
    # [comment] average outlier token number in all samples, and round up
    # [comment] maximum token count among all layers is used as final outlier number
    ...
    # [comment] BOS and high frequency outliers are prefixed
