...
class UniformAffineQuantizer(nn.Module):
    def __init__(
        ...
        self.group_size = group_size if group_size != -1 else quantized_shape[-1] # [comment] per tensor by default
        assert quantized_shape[-1] % group_size == 0
        self.inc_groups = quantized_shape[-1] // self.group_size # [comment] number of quantization groups. 1 if per tensor
    ...
    def static_fake_quant(self, x, fr=False):
        '''
        static quantization
        '''
        ...
        if fr:
            x_fr = round_ste(x_reshaped / scale).abs() / self.qmax
            # print(f'[debug] fr: {x_fr.mean()}')
    ...
    def forward(self, x: torch.Tensor, fr=False):
        if self.n_bits >= 16 or not self.enable:
            return x

        if self.mode == 'static':
            x_dequant = self.static_fake_quant(x, fr)
        elif self.mode == 'dynamic':
...
class SoftmaxQuantizer(UniformAffineQuantizer):
    ...
    def static_fake_quant(self, x, fr=False):
        '''
        static quantization x.shape = (bs, h, n, n)
        scale.shape = (h, 1)
        '''
        scale = clamp_ste(self.scale, 1e-4, 1e4)
        round_zero_point = clamp_ste(round_ste(self.zero_point), self.qmin, self.qmax) if self.zero_point is not None else None

        x_int = round_ste(x / scale.unsqueeze(-1))
        if round_zero_point is not None:
            x_int = x_int.add(round_zero_point)
        x_int = x_int.clamp(self.qmin, self.qmax)
        x_dequant = x_int
        if round_zero_point is not None:
            x_dequant = x_dequant.sub(round_zero_point)
        x_dequant = x_dequant.mul(scale.unsqueeze(-1))

        # print(f'[debug] loss: {(x - x_dequant).pow(2).mean(dim=(0,2,3))}')
        return x_dequant
    ...
    def find_activation_quant_param(self, quantized_item_stat, minmax_init):
        if self.mode == 'static':
            if minmax_init:
                # x = quantized_item_stat.reshape(-1,self.group_size)
                # xmax = x.amax([-1], keepdim=True)   
                xmax = quantized_item_stat.unsqueeze(-1)
                scale = xmax/(2**self.n_bits-1)
                scale = scale.clamp(min=1e-4, max=1e4)
                self.scale = nn.Parameter(scale)
                if self.asym:
                    zero_point = (2**(self.n_bits-1)-1)*torch.ones_like(self.scale)
